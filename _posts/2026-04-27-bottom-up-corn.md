...
So the better first question is not:

$$
\text{Will corn go up or down tomorrow?}
$$

The better first question is:

$$
\text{What physical state is the corn market in today?}
$$

This change in question changes the whole modeling problem. A futures price becomes the market’s attempt to value the corn balance sheet through time. It reflects expected supply, expected demand, inventories, storage costs, scarcity, delivery timing, and the specific contract month being priced.

This matters because the futures curve is not one single corn price. A nearby contract can be driven by old crop inventory, immediate availability, and nearby scarcity. A deferred contract can be driven by acreage, yield, harvest expectations, and new crop supply. The same information can therefore have different effects depending on which contract is being priced.

The strategy begins from that precise principle:
...
The model does not begin by asking whether corn should rise or fall tomorrow. It first asks what physical state the market is in, then determines what each futures contract should be worth in that state. Only after that does it decide whether the observed market price is cheap or expensive enough to justify a trade.

## The economic decomposition

The trading problem can be separated into three connected layers. The first layer estimates the physical state of the corn market, including supply, demand, inventories, stocks to use, positioning, seasonality, and risk conditions. The second layer translates that state into a fair value for each futures contract, because a July contract and a December contract do not represent the same economic exposure. July is more closely linked to old crop inventory and nearby scarcity, while December is more closely linked to new crop acreage, yield, and harvest expectations. The third layer trades only when the gap between market price and fitted fair value is large enough to compensate for model error, execution cost, and risk.

This gives the core identity

$$
\text{Corn Trading} = \text{State Estimation} + \text{Price Translation} + \text{Risk Controlled Execution}.
$$
...
A futures contract is not a generic corn price. It is a claim on deliverable corn during a specific delivery month. Each contract therefore has three layers of meaning.
...
This is why the strategy trades both outright contracts and calendar spreads. Outrights express contract level fair value errors, while spreads express relative curve fair value errors.

## The balance sheet foundation

The physical corn market begins with accounting. The market first has a quantity of corn available, which comes from beginning stocks, production, and imports. Adding these components gives total supply:

$$
\text{TotalSupply}_t = \text{BeginningStocks}_t + \text{Production}_t + \text{Imports}_t.
$$

Corn then disappears through use. The major use categories are exports, ethanol use, other food, seed, and industrial use, and feed and residual use. Adding these demand categories gives total use:

$$
\text{TotalUse}_t = \text{Exports}_t + \text{EthanolUse}_t + \text{OtherFSI}_t + \text{FeedResidual}_t.
$$
...
Ending stocks alone are not enough to measure tightness. The same inventory level can be loose when demand is low and tight when demand is high. What matters is the inventory cushion relative to the rate at which corn is being used. The inventory cushion is ending stocks, and the demand base is total use. The normalized tightness measure is therefore

$$
\text{StocksToUse}_t = \frac{\text{EndingStocks}_t}{\text{TotalUse}_t}.
$$

This ratio is central because it connects supply and demand in one number. If ending stocks rise while total use is unchanged, the ratio rises and the market becomes looser. If total use rises while ending stocks are unchanged, the ratio falls and the market becomes tighter.

Because higher stocks to use means a larger inventory cushion, fair value should move in the opposite direction:

$$
\frac{\partial \text{FairValue}_t}{\partial \text{StocksToUse}_t} < 0.
$$

Because higher production increases available supply, fair value should also move in the opposite direction of production:

$$
\frac{\partial \text{FairValue}_t}{\partial \text{Production}_t} < 0.
$$

Because higher demand reduces ending stocks, fair value should move in the same direction as demand:

$$
\frac{\partial \text{FairValue}_t}{\partial \text{Demand}_t} > 0.
$$

These signs are not fitted coefficients yet. They are economic sign restrictions used when raw physical information is transformed into factor signals.
...
The components do not arrive at the same frequency. Futures prices are daily, export and ethanol information can be weekly, balance sheets can be monthly, and acreage and production information can be seasonal. The implementation therefore creates a daily panel where each row represents the information that was available on that date.

## Physical forecast modules

The physical modules estimate physical quantities before any price model is applied. This separation is essential. The acreage model estimates acres, the yield model estimates yield, and the export model estimates exports. None of these modules directly estimates a futures price.

The price effect appears only after the physical estimate has been economically signed, standardized, combined into a factor, and passed into the contract level pricing model. The physical layer therefore follows this chain:

$$
\text{Physical Inputs} \rightarrow \text{Physical Estimates} \rightarrow \text{Balance Sheet} \rightarrow \text{Economic Signals}.
$$

Only after this step does the model move toward price.

## Acreage module

Acreage begins from a prior expectation. The prior represents the baseline planted acreage estimate available before the model adjustment. The model then adds information that may not be fully reflected in the prior, such as relative corn and soybean economics, planting delays, fieldwork conditions, soil moisture, and state level planting behavior.

The model estimate is built from a prior plus an adjustment:

$$
\text{PlantedAcres}_t = \text{USDAPrior}_t + \text{ModelAdjustment}_t.
$$

The economic sign follows from the supply channel. More planted acres create a larger potential production base, which loosens the balance sheet. Higher than prior planted acreage is therefore bearish, while lower than prior planted acreage is bullish.

## Harvested acres module

Planted acres are not the same as harvested grain acres. Some acres can be abandoned, some can be used as silage, and some can be affected by weather. The model must therefore estimate the share of planted acres that becomes harvested grain acres.

That share is the harvested ratio. It begins from a historical baseline and is adjusted for weather and residual effects:

$$
\text{HarvestedRatio}_t = \text{HistoricalBaseline}_t + \text{WeatherShock}_t + \text{Residual}_t.
$$

Once the harvested ratio is known, harvested acres are obtained by applying that ratio to planted acres:

$$
\text{HarvestedAcres}_t = \text{PlantedAcres}_t \times \text{HarvestedRatio}_t.
$$

The economic sign again follows from production. Higher harvested acres increase the area that can produce grain and are therefore bearish. Lower harvested acres reduce expected grain supply and are bullish.

## Yield module

Yield must be modeled relative to expectation. Raw yield is not directly comparable across long histories because farming technology, seed genetics, machinery, agronomy, and management practices improve over time. A yield that was high decades ago may be normal or low today.

The model therefore separates yield into a trend component and a crop year deviation component. The trend component captures structural improvement, while the deviation component captures whether the current crop is performing better or worse than expected.

Planting pace, crop condition, growing degree days, drought, and heat stress can all move yield away from trend. The yield estimate is built by starting from trend yield and adding these deviations:

$$
\text{Yield}_t = \text{TrendYield}_t + \beta_1\text{PlantingPaceAnomaly}_t + \beta_2\text{CropConditionAnomaly}_t + \beta_3\text{GDDAnomaly}_t + \beta_4\text{DroughtAnomaly}_t + \beta_5\text{HeatStressProxy}_t + \varepsilon_t.
$$
...
Production is not estimated independently after harvested acres and yield have been estimated. It is the quantity of grain produced per harvested acre multiplied by the number of harvested acres:

$$
\text{Production}_t = \text{HarvestedAcres}_t \times \text{Yield}_t.
$$

This identity keeps the balance sheet internally consistent. If the model changes yield, production must change. If the model changes harvested acres, production must also change.
...
Exports are observed gradually. Waiting only for monthly balance sheet updates ignores useful weekly information. Export sales, export inspections, competitor supply, and destination demand can all indicate whether exports are running ahead of or behind expectations.

The export model starts from a prior and adds pace related adjustments:

$$
\text{Exports}_t = \text{USDAPrior}_t + \gamma_1\text{SalesPaceGap}_t + \gamma_2\text{InspectionPaceGap}_t + \gamma_3\text{CompetitorSupplyGap}_t + \gamma_4\text{DestinationDemandSignal}_t + \varepsilon_t.
$$

The economic sign is the opposite of supply. Stronger exports increase total use. Higher total use lowers ending stocks, all else equal, so stronger export pace is bullish.

## Ethanol module

Ethanol is a major corn demand category. Weekly ethanol output provides a higher frequency signal than monthly balance sheets, so the model starts with a prior ethanol use estimate and adjusts it using weekly output and seasonal information.

The ethanol estimate is

$$
\text{EthanolUse}_t = \text{USDAPrior}_t + \delta_1\text{EthanolOutputGap}_t + \delta_2\text{SeasonalFactor}_t + \varepsilon_t.
$$

Higher ethanol output implies more corn used for ethanol. More use tightens the balance sheet, so higher ethanol use is bullish.

## Feed and residual module

Feed and residual is harder to observe at high frequency. It is an important demand category, but it is partly residual by construction. The model should therefore not pretend to have the same precision here as it has for more directly observed series such as exports or ethanol.

The feed and residual estimate is built conservatively:

$$
\text{FeedResidual}_t = \text{SmoothedUSDAPrior}_t + \text{LivestockFeedAdjustment}_t + \text{LargeUncertainty}_t.
$$

Higher feed and residual use increases total use and tightens ending stocks. The sign is bullish, but the uncertainty is larger.

## From physical estimates to economic factors

The physical estimates are measured in different units. Acreage is measured in acres, yield in bushels per acre, production and stocks in bushels, stocks to use as a ratio, positioning in contracts, and returns as log price changes. These quantities cannot be averaged directly.

The model must first make them directionally consistent and then make them scale comparable. The transformation has three stages:
...
$$
z_t > 0 \quad \Rightarrow \quad \text{more bullish, tighter, or stronger than normal}.
$$
...
The supply factor answers one question:
...
For each supply component, the model compares a prior estimate with an updated estimate. Across the supply block, the economic direction is the same: more expected supply is bearish, while less expected supply is bullish. Because the factor should rise when supply is tighter, the signed signal must be positive when the model estimate is below the prior.
...
Production receives the largest construction weight because it enters the balance sheet directly. Yield receives a large weight because it is often the dominant in season uncertainty. Planted acreage and harvested acreage receive smaller weights because they affect supply through production. The weights sum to one:

$$
0.40 + 0.20 + 0.15 + 0.25 = 1.
$$

The supply factor is the weighted standardized measure of supply tightness:

$$
\text{SupplyFactor}_t = 0.40z^{\text{prod}}_t + 0.20z^{\text{acreage}}_t + 0.15z^{\text{harvested}}_t + 0.25z^{\text{yield}}_t.
$$

A positive supply factor means expected supply is tighter than normal after comparing model estimates with priors and standardizing each component.

## Balance factor

The balance factor answers:
...
$$
\text{BalanceFactor}_t = 0.50z^{\text{stu}}_t + 0.35z^{\text{revision}}_t + 0.15z^{\text{ending}}_t.
$$

## Demand factor

The demand factor answers:
...
The export pace ratio is accumulated exports divided by expected accumulated exports:

$$
\frac{\text{AccumulatedExports}_t}{\text{SeasonalExpectedExports}_t}.
$$
...
$$
\text{DemandFactor}_t = 0.35z^{\text{exports}}_t + 0.30z^{\text{ethanol}}_t + 0.20z^{\text{feed}}_t + 0.15z^{\text{totaluse}}_t.
$$

## Positioning factor

The positioning factor answers:
...
The positioning factor is

$$
\text{PositioningFactor}_t = 0.70z^{\text{level}}_t + 0.30z^{\text{change}}_t.
$$
...
$$
\text{CrowdingRisk}_t = \left|z^{\text{level}}_t\right|.
$$
...
The sine coordinate is

$$
\text{SeasonSin}_t = \sin(\theta_t),
$$

or equivalently,

$$
\text{SeasonSin}_t = \sin\left(2\pi\frac{\text{MYDay}_t}{365}\right).
$$

The cosine coordinate is

$$
\text{SeasonCos}_t = \cos(\theta_t),
$$

or equivalently,

$$
\text{SeasonCos}_t = \cos\left(2\pi\frac{\text{MYDay}_t}{365}\right).
$$

Both coordinates are needed. One coordinate alone cannot uniquely locate a point on a circle. Together, sine and cosine encode the crop calendar position smoothly without creating an artificial jump between the end and beginning of the year.
...
$$
\text{SeasonalityFactor}_t = 0.20\text{PlantingRegime}_t + 0.45\text{WeatherYieldRegime}_t + 0.20\text{HarvestRegime}_t + 0.15\text{PostHarvestDemandRegime}_t.
$$
...
The risk factor answers:

> Is the market environment calm, trending, volatile, or shocked?

The physical balance sheet may point in one direction, but the reliability of a valuation residual depends on market conditions. A residual in a calm market is not the same as a residual during a high volatility shock.
...
$$
\text{RiskFactor}_t = 0.45z^{\text{mom}}_t + 0.40z^{\text{vol}}_t + 0.15z^{\text{shock}}_t.
$$

## Economic anchor

Before fitting contract prices, it is useful to have one broad measure of fundamental pressure. The anchor combines the larger factor groups. It is not the trading model; it is a summary state variable.
...
$$
\text{FundamentalAnchor}_t = 0.30\text{SupplyFactor}_t + 0.35\text{BalanceFactor}_t + 0.10\text{DemandFactor}_t + 0.10\text{PositioningFactor}_t + 0.05\text{SeasonalityFactor}_t + 0.10\text{RiskFactor}_t.
$$

The distinction between construction weights and pricing weights is important:

$$
\text{Anchor Weights} \ne \text{Pricing Weights}.
$$
...
Before the factors are used for pricing, the first check is whether they actually move over time. If the factor panel is constant, missing, or mechanically duplicated, the pricing model cannot learn useful relationships.
...
The second check is whether the factors are too correlated. Some correlation is natural: yield, production, ending stocks, and stocks to use are all connected through the same balance sheet. The goal is not zero correlation. The goal is stable estimation.
...
The model must explain the log price of a specific contract. A contract price depends on the fundamental factor state, the raw physical state, the contract’s location on the curve, and interactions between the state and the contract.

The feature vector is therefore built from four blocks:

$$
X_{t,k} = [\text{Factor State}, \text{Physical State}, \text{Curve State}, \text{Interactions}].
$$

The factor state answers how unusual the current corn market state is relative to history. The physical state captures the actual level of the balance sheet. The curve state identifies which contract is being priced. The interactions allow the same economic factor to affect different contracts differently.
...
Then convert that fraction into an angle on a full circle:

$$
\phi_k = 2\pi v_k.
$$

Substituting the month fraction gives

$$
\phi_k = 2\pi\frac{m_k}{12}.
$$

The sine coordinate of the delivery month is

$$
\text{MonthSin}_k = \sin(\phi_k),
$$

or

$$
\text{MonthSin}_k = \sin\left(2\pi\frac{m_k}{12}\right).
$$

The cosine coordinate is

$$
\text{MonthCos}_k = \cos(\phi_k),
$$

or

$$
\text{MonthCos}_k = \cos\left(2\pi\frac{m_k}{12}\right).
$$
...
To allow the supply effect to depend on maturity, add a term that multiplies supply by days to expiry:

$$
\theta_S\left(\text{SupplyFactor}_t \times \text{DTE}_{t,k}\right).
$$

The supply related part of the model is

$$
\beta_S\text{SupplyFactor}_t + \theta_S\left(\text{SupplyFactor}_t \times \text{DTE}_{t,k}\right).
$$

Collecting the common factor gives
...
The same logic applies to delivery month sensitivity. Start with a direct balance effect:

$$
\beta_B\text{BalanceFactor}_t.
$$

Then allow the balance effect to vary smoothly by delivery month using both cyclical month coordinates:

$$
\theta_{B,\sin}\left(\text{BalanceFactor}_t \times \text{MonthSin}_k\right) + \theta_{B,\cos}\left(\text{BalanceFactor}_t \times \text{MonthCos}_k\right).
$$

The balance related part of the model is

$$
\beta_B\text{BalanceFactor}_t + \theta_{B,\sin}\left(\text{BalanceFactor}_t \times \text{MonthSin}_k\right) + \theta_{B,\cos}\left(\text{BalanceFactor}_t \times \text{MonthCos}_k\right).
$$

Collecting the common balance factor gives

$$
\left(\beta_B + \theta_{B,\sin}\text{MonthSin}_k + \theta_{B,\cos}\text{MonthCos}_k\right)\text{BalanceFactor}_t.
$$

The effective balance sensitivity is therefore

$$
\frac{\partial \ln(F_{t,k})}{\partial \text{BalanceFactor}_t} = \beta_B + \theta_{B,\sin}\text{MonthSin}_k + \theta_{B,\cos}\text{MonthCos}_k.
$$
...
$$
y_{t,k} = \alpha_t + X_{t,k}^{\top}\beta_t + \varepsilon_{t,k}.
$$

Since the target is the log futures price,

$$
\ln(F_{t,k}) = \alpha_t + X_{t,k}^{\top}\beta_t + \varepsilon_{t,k}.
$$
...
$$
\text{TrainingSet}_{t+1} = \text{TrainingSet}_t \cup \{(X_t,y_t)\}.
$$
...
$$
\text{FairValue}_{t,k} = \exp(\hat{y}_{t,k}).
$$

The market price can now be compared with the fitted fair value. An expensive contract satisfies

$$
F^{\text{market}}_{t,k} > \text{FairValue}_{t,k}.
$$

A cheap contract satisfies
...
The pricing z score is the current residual divided by the past residual scale:

$$
\text{PricingZ}_{t,k} = \frac{\text{Residual}_{t,k}}{\hat{\sigma}_{\text{resid},t}}.
$$
...
$$
\text{SpreadUncertainty}_t = \max\left(\sqrt{(0.5F^{\text{fair}}_{t,n}\sigma_n)^2 + (0.5F^{\text{fair}}_{t,f}\sigma_f)^2},4.0\right).
$$

The spread z score is

$$
\text{SpreadZ}_t = \frac{\text{SpreadError}_t}{\text{SpreadUncertainty}_t}.
$$

A future improvement should estimate spread residual volatility directly instead of relying on a leg level approximation.

## Outright trading rule

The outright trading rule follows the sign of the pricing z score. If the pricing z score is positive, the market price is above fitted fair value and the contract is expensive relative to the model. A sufficiently positive z score creates a short signal:
...
$$
|\text{PricingZ}_{t,k}| < 0.30.
$$

## Spread trading rule

The spread rule follows the sign of the spread z score. If the spread z score is positive, the market spread is above fitted fair spread. The near contract is expensive relative to the far contract, so the strategy shorts the near leg and buys the far leg:
...
On a given date, the model may find both outright and spread opportunities. Each opportunity is scored by the absolute size of its z score:
...
$$
\text{Score}_{\text{spread}} \ge 0.90 \cdot \text{Score}_{\text{outright}}.
$$
...
$$
\text{PositionSize}_t \propto \frac{|Z_t|}{\text{ModelUncertainty}_t}.
$$
...
The implementation connects the research state file to the futures chain. At each date, the algorithm reads the available state, obtains the current futures chain, builds contract level features, estimates fair value, computes mispricing scores, decides whether to trade, and only then updates the training set.
...
<tr><td>Winning trades</td><td>22</td></tr>
<tr><td>Losing trades</td><td>13</td></tr>
<tr><td>Win rate</td><td>62.86%</td></tr>
...
$$
\text{PayoffRatio} = \frac{465.91}{901.92} \approx 0.5166.
$$
...
$$
E[\text{PnL}] = P(\text{win})\cdot\text{AverageWin} + P(\text{loss})\cdot\text{AverageLoss}.
$$
...
$$
0.6286\cdot465.91 \approx 292.80.
$$
...
The second limitation is spread uncertainty. The current implementation approximates spread uncertainty from leg uncertainty. A stronger method would estimate spread residual volatility directly from historical spread errors.
...
The fourth limitation is event risk. Major USDA reports can create jumps, so the model should apply special exposure controls around WASDE, Grain Stocks, Prospective Plantings, Acreage, and Crop Production reports.
...
The strategy has a coherent economic structure. It begins with the physical corn balance sheet, transforms physical information into signed and standardized economic factors, estimates contract level fair value with a no lookahead ridge model, and trades residual mispricing only when the signal is large enough.
...
The first implementation is not yet profitable, but it shows that the pipeline works technically. The core empirical result is

$$
\text{Win Rate} > 50\%, \qquad |\text{Average Loss}| > \text{Average Win}.
$$
