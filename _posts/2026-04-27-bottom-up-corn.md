...
This gives the economic identity:
...
Therefore, the state variable at date \(t\) is not the final historical database. It is the information set available at that time,
...
For that reason, the model must estimate,
...
The cost side of carry is financing plus storage. The benefit side is convenience yield. The net carry rate is therefore built as

$$
\text{NetCarry}_t = r_t + c_t - y_t.
$$

note that a positive net carry means deferred corn should be more expensive than nearby corn because carrying inventory is costly. A lower or negative net carry means nearby corn can become relatively more valuable because physical scarcity raises convenience yield.

If corn is carried from date \(t\) to maturity \(T\), the carry period is,

$$
T - t.
$$

where a continuously compounded carry rate over that period creates the carry multiplier:
...
A futures price for maturity \(T\) can then be approximated by taking the current spot or nearby reference value \(S_t\) and applying the carry multiplier:
...
The log of the futures price is approximately the log of the nearby reference price plus the carry term:
...
A spread is built by subtracting the far contract from the near contract:
...
The storage intuition can therefore be written as:
...
Beginning stocks are the inventory carried into the period. Production is the new crop supply. Imports complete the supply identity. Adding these pieces gives total supply:
...
Adding these demand categories gives total use:
...
Substituting the supply and use components into the ending stocks identity gives the full balance sheet relation:
...
The inventory cushion is ending stocks. The demand base is total use. The normalized tightness measure is therefore built by dividing ending stocks by total use:
...
The model estimate is therefore built from a prior plus an adjustment:
...
Planting pace, crop condition, growing degree days, drought, and heat stress are observable variables that can move yield away from trend. The yield estimate is built by starting from trend yield and adding those deviations:
...
The trading relevant yield quantity is the difference between model yield and trend or prior yield. That difference is built as:
...
Production is the quantity of grain produced per harvested acre multiplied by the number of harvested acres. The physical identity is therefore:
...
The ethanol estimate is built as:
...
The feed and residual estimate is therefore built conservatively:
...
If a higher value is bullish, the signed signal keeps the raw sign:
...
Start with production. Production is the direct new crop supply addition in the balance sheet. If model production is below prior production, expected supply is smaller than previously believed. That is bullish. The production tightness signal is therefore prior production minus model production:

$$
s^{\text{prod}}_t = \text{PriorProduction}_t - \text{ModelProduction}_t.
$$

The same object can be built in two steps. First measure the model revision relative to the prior:
...
Planted acres define the potential production base. If the model estimates fewer planted acres than the prior, potential supply is smaller. That is bullish. The planted acreage tightness signal is therefore:

$$
s^{\text{acreage}}_t = \text{PriorPlantedAcres}_t - \text{ModelPlantedAcres}_t.
$$

Equivalently, the raw acreage surprise is model planted acres minus prior planted acres:
...
Harvested acres are closer to production than planted acres because they represent the acres that actually contribute grain. If the model estimates fewer harvested acres than the prior, expected grain supply is smaller. That is bullish. The harvested acreage tightness signal is:
...
Yield must be measured relative to trend or prior expectation. If model yield is below the expected yield level, production falls. That is bullish. The yield tightness signal is therefore expected yield minus model yield:
...
The weights are chosen to sum to one:
...
The most compact tightness variable is stocks to use. Higher stocks to use means more inventory cushion relative to demand. That is bearish. Therefore the signed stocks to use signal must rise when stocks to use falls:
...
The change in stocks to use is:
...
Ending stocks also matter as an inventory level. Higher ending stocks mean more remaining supply. That is bearish. The signed ending stocks signal is:
...
The absolute value of the standardized level does that:
...
Let the marketing year day at date \(t\) be:

$$
\text{MYDay}_t \in \{1,2,\ldots,365\}.
$$

The first step is to convert the day into a fraction of the annual cycle:

$$
u_t = \frac{\text{MYDay}_t}{365}.
$$

A full circle has \(2\pi\) radians. To convert the fraction of the marketing year into an angle on that circle, multiply by \(2\pi\):
...
Substituting the crop calendar angle gives:
...
Substituting the same angle gives:
...
The regime based seasonality factor is:
...
A log return compares today’s price with yesterday’s price in logarithmic space:
...
A return shock measures whether today’s return is unusual relative to recent returns. The recent mean must be computed using only earlier returns:
...
The futures chain contains multiple contracts:
...
Let the delivery month of contract \(k\) be:
...
Convert that fraction into an angle on a full circle:
...
Substituting the delivery month angle gives:

$$
\text{MonthSin}_k = \sin\left(2\pi\frac{m_k}{12}\right).
$$

The cosine coordinate of the delivery month is:
...
Start with a direct supply effect. If supply tightness increases, log price may change by a coefficient \(\beta_S\):
...
The supply related part of the model is the sum of these two terms:

$$
\beta_S\text{SupplyFactor}_t + \theta_S\left(\text{SupplyFactor}_t \times \text{DTE}_{t,k}\right).
$$

Both terms contain \(\text{SupplyFactor}_t\), so the common factor can be collected:
...
Start with a direct balance effect:

$$
\beta_B\text{BalanceFactor}_t.
$$

Allow the balance effect to vary smoothly by delivery month using both cyclical month coordinates:
...
Since the target is the log futures price, substitute \(y_{t,k} = \ln(F_{t,k})\):
...
For observation \(i\), the model prediction is the feature vector multiplied by the coefficient vector:
...
Substituting the fitted value into the error gives:
...
The training set at date \(t\) contains only observations before date \(t\). Therefore the total historical squared pricing error is:
...
For \(p\) features, the total coefficient size is measured by the sum of squared coefficients:

$$
\sum_{j=1}^{p}\beta_j^2.
$$

The strength of the penalty is controlled by the shrinkage parameter \(\lambda\). Multiplying the coefficient size term by \(\lambda\) gives the ridge penalty:

$$
\lambda\sum_{j=1}^{p}\beta_j^2.
$$

The ridge objective combines the historical pricing error and the coefficient penalty:
...
The fitted log price for contract \(k\) is:
...
An expensive contract satisfies:
...
The market log price is:

$$
\ln(F^{\text{market}}_{t,k}).
$$

The fitted fair log price is:
...
For near contract \(n\) and far contract \(f\), the observed market spread is:
...
The implemented short threshold is:
...
The implemented long threshold is:
...
The outright exit condition is:
...
The positive spread entry condition is:
...
The negative spread entry condition is:
...
Each opportunity is scored by the absolute size of its z score:
...
For an outright trade, the unit is:
...
The signal strength is the absolute z score:

$$
|Z_t|.
$$

The model uncertainty is the scale of residual error, volatility, or another risk estimate:
...
The win rate is winners divided by total closed trades:
...
The payoff ratio compares the average win to the absolute average loss:
